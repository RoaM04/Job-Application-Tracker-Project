...
| TC-08 | `update_application_status` | Simulate a slow or unavailable file operation that exceeds the configured timeout.          | Attempt to update an application's status while the file operation is unavailable or times out. | A timeout/error response is returned and the tool does not hang indefinitely.                                 | **To be completed during manual execution.**                                                                                                                                                     | [TC-08 screenshot](evidence/TC-08-update-status-timeout.png) |
...
After completing the empty-data test, restore the normal fixture data.
